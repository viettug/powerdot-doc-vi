...
của \pf{pstrics} \cite{PSTricksWeb,PSTricks}. Giá trị của |logopos| xác định
vị trí |logo|; tuỳ chọn |logocmd| dùng để định nghĩa |logo| (ví dụ nạp một hình 
vẽ); cuối cùng, |logohook| dùng để xác định độ lệch của |logo|,
...
Dùng |false| nếu bạn muốn tắt hiệu ứng này.
...
\begin{example}
 \pdsetup{
   dminwidth=.1\slidewidth,maxwidth=.9\slidewidth,
   dminheight=.2\slideheight,maxheight=.8\slideheight
 }
\end{example}
...
\begin{example}
 \pdsetup{
   randomdots,dots={dotstyle=osquare}
 }
\end{example}
...
\begin{example}
 \pdsetup{
   randomdots,dminsize=500pt,dmaxsize=600pt,dmindots=2,
   dmaxdots=5,dots={dotstyle=ocircle,linewidth=.5pt}
 }
\end{example}
